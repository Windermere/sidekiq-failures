  ...
  module Failures
    describe "Middleware" do
      before do
        ...
        Sidekiq1server_middleware {|chain| chain.add Sidekiq1::Failures::Middleware }
        Sidekiq1redis = REDIS
        Sidekiq1redis { |c| c.flushdb }
        Sidekiq1instance_eval { @failures_default_mode = nil }
      end

      TestException = Class.new(Exception)
      ShutdownException = Class.new(Sidekiq1::Shutdown)
      ...
      it 'raises an error when failures_default_mode is configured incorrectly' do
        assert_raises ArgumentError do
          Sidekiq1failures_default_mode = 'exhaustion'
        end
      end

      it 'defaults failures_default_mode to all' do
        assert_equal :all, Sidekiq1failures_default_mode
      end
      ...
      it "doesn't record failure if going to be retired again and configured to track exhaustion by default" do
        Sidekiq1failures_default_mode = :exhausted
      ...
      it "removes old failures when failures_max_count has been reached" do
        assert_equal 1000, Sidekiq1failures_max_count
        Sidekiq1failures_max_count = 2
        ...
        Sidekiq1failures_max_count = false
        assert Sidekiq1failures_max_count == false

        Sidekiq1failures_max_count = nil
        assert_equal 1000, Sidekiq1failures_max_count
      end
      ...
      it 'returns the total number of failed jobs in the queue' do
        ...
        assert_equal 0, Sidekiq1::Failures.count
        ...
        assert_equal 1, Sidekiq1::Failures.count
      end

      def failures_count
        Sidekiq1redis { |conn| conn.zcard(LIST_KEY) }
      end

      def create_work(msg)
        Sidekiq1::BasicFetch::UnitOfWork.new('default', Sidekiq1dump_json(msg))
      end
    end
  end
end
